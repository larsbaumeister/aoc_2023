...
impl Day3 {
    ...
    fn solve() -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..matrix.len() {
            for j in 0..matrix[i].len() {
                // this is for part 1
                if Day3::is_symbol(matrix[i][j]) {
                    let res = Day3::find_adjesent_numbers(&matrix, Point { i: i, j: j });
                    number_positions.extend(res);
                }
    
                // this is for part 2
                if matrix[i][j] == b'*' {
                    let mut res = Day3::find_adjesent_numbers(&matrix, Point { i: i, j: j });
                    res.sort();
                    res.dedup();
                    if res.len() == 2 {
                        let num1 = Day3::read_number(&matrix, res[0]);
                        let num2 = Day3::read_number(&matrix, res[1]);
        ...
        let res: u32 = number_positions.into_iter()
            .map(|p| Day3::read_number(&matrix, p))
            .sum();
    ...
    fn find_adjesent_numbers(matrix: &Vec<Vec<u8>>, point: Point) -> Vec<Point> {
        ...
        if Day3::is_digit(matrix[max(point.i - 1, 0)][point.j]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i -1, j: point.j }));
        }

        if Day3::is_digit(matrix[min(point.i + 1, matrix.len())][point.j]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i +1, j: point.j }));
        }

        if Day3::is_digit(matrix[point.i][max(point.j - 1, 0)]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i, j: point.j -1 }));
        }
        
        if Day3::is_digit(matrix[point.i][min(point.j + 1, matrix[point.i].len())]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i, j: point.j +1 }));
        }

        if Day3::is_digit(matrix[max(point.i - 1, 0)][max(point.j - 1, 0)]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i -1, j: point.j -1 }));
        }

        if Day3::is_digit(matrix[max(point.i - 1, 0)][min(point.j + 1, matrix[point.i].len())]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i -1, j: point.j +1 }));
        }

        if Day3::is_digit(matrix[min(point.i + 1, matrix.len())][max(point.j - 1, 0)]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i +1, j: point.j -1 }));
        }

        if Day3::is_digit(matrix[min(point.i + 1, matrix.len())][min(point.j + 1, matrix[point.i].len())]) {
            starting_points.push(Day3::find_starting_point_of_number(matrix, Point { i: point.i +1, j: point.j +1 }));
        }
    ...
    fn find_starting_point_of_number(matrix: &Vec<Vec<u8>>, point: Point) -> Point {
        ...
        while starting_point.j > 0 && Day3::is_digit(matrix[starting_point.i][starting_point.j -1]) {
            starting_point.j -= 1;
        }
        starting_point
    }
    ...
    fn read_number(matrix: &Vec<Vec<u8>>, point: Point) -> u32 {
        ...
        let i = point.i;
        let mut j = point.j;
        while j < matrix[i].len() && Day3::is_digit(matrix[i][j]) {
            number *= 10;

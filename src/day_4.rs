...
impl Day4 {
    ...
    pub fn solve() -> Result<(), Box<dyn Error>> {
        ...
        for line in reader.lines() {
            let line = line?;
            let (_, points) = Day4::calc_line(&line);
            total_win += points;
            lines.push(line);
        }
    
        let (wins, _) = Day4::version_2(&lines, 0, lines.len());
    ...
    fn version_2(lines: &Vec<String>, start: usize, end: usize) -> (u32, u32) {
        ...
        for i in start..end {
            let (number_of_wins, points) = Day4::calc_line(&lines[i]);
            total_wins += number_of_wins;
            total_points += points;
    
            let (number_of_wins, _) = Day4::version_2(&lines, i + 1, min(i + 1 + number_of_wins as usize, lines.len()));
            total_wins += number_of_wins;
        }
